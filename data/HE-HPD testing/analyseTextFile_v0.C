...
#include <fstream>
#define npoints 25
#define input_file "HE-HPD_22012018_1_combined_v2_1.txt"
//-----------------------------------------------
...
void fitGraph(std::vector<std::string> diode_pos){
  ...
  for(int i =0; i<diode_pos.size(); i++){
    ...
    string outname1(diode_pos[i]+".C");
    can->SaveAs(outname.c_str());
    can->SaveAs(outname1.c_str());
  }
}

void plotFitParam(std::vector<std::string> diode_pos){
 TFile *rf = new TFile("HE_IV_25all.root","recreate");
  ...
  ///////////////////////////// 1st Canvas /////////////////////////// 
  for(int i =0; i<diode_pos.size(); i++){
     cout<<diode_pos[i]<<"\t"<<std::setprecision(4)<<intX_vec[i]<<"\t"<<std::setprecision(4)<<intY_vec[i]<<"\t"<<Iat7kV[i]<<"\t"<<Iat7kV_2[i]<< "\t" << std::setprecision(4) << m_fit_2[i] << endl;
  }
  TCanvas *can1 = new TCanvas();
  can1->Divide(1, 3);
  can1->cd(1);
  TGraph* gr_Iat7kV_Vbr = makeGraph(intX_vec, intY_vec);
  decorateGraph(gr_Iat7kV_Vbr, "Breakdown Voltage (kV)", "Current(nA) at Breakdown", " ", 150, 300, 3);  
  ...
  //----------------------------
  can2->cd(2);
  TH1F* h_intX = makeHisto(intX_vec, "Breakdown Voltage (V_{BR})", "Voltage (kV)", "Entries",  40,  3, 4, 2);
  h_intX->Draw("HIST");
  h_intX->Write("HIST");
  ...
  can2->cd(8);
  TH1F* h_m2 = makeHisto(m_fit_2, "Resistance", "Resistance (G#Omega)", "Entries",  40,  20, 60, 40);
  h_m2->Draw("HIST");
  h_m2->Write("Resistance_HIST");
...
std::vector<Double_t> getDiodeCol(string diode_position, string col_name){
  //open input file
  ifstream infile;
  infile.open(input_file);//open the text file
  ...
  Int_t Date_; 
  Double_t Time_; 
  Double_t X_, Y_, Z_, DCurrent_, HV_, I_, I_r;
