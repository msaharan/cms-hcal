...
void fitGraph(std::vector<std::string> diode_pos){
  ...
  vector<double>pos;
  TF1 *fit_line_1 = new TF1("fit", "[0]*x +[1]", 0, 2);
  TF1 *fit_line_2 = new TF1("fit", "[0]*x +[1]", 5.3, 7);
//  TF1 *fit_line_2 = new TF1("fit", "[0]*x +[1]", 3.5, 7);
  for(int i =0; i<diode_pos.size(); i++){
    ...
    myLable2<<"Fit Range = (5.3, 7):"<<" 1/m= "<<std::setprecision(4)<<1000/m_2<<" G#Omega"<<", c="<<c_2<<" nA"<<endl;
...
cout<<"Hello "<<i<<endl;
  ...
  avg_volt = avg_volt+intX_vec[i];
  }
  for(int i=0;i<25;i++)
  {
	  fout<<intX_vec[i]<<endl;
	}
	fout.close();
	 
  cout<<"avg_volt "<< avg_volt/25<<endl;
